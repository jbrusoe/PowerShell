...
#[float] forces the variables to become floating point numbers instead
#of strings (Read-Host default return type)
[float]$FirstNumber = Read-Host "Enter the first number"
[float]$SecondNumber = Read-Host "Enter a second number"

Write-Output $("$FirstNumber + $SecondNumber = " + $($FirstNumber+$SecondNumber))
Write-Output $("$FirstNumber - $SecondNumber = " + $($FirstNumber-$SecondNumber))
Write-Output $("$FirstNumber * $SecondNumber = " + $($FirstNumber*$SecondNumber))
...
if ($SecondNumber -eq 0) {
	...
	Write-Output $("$FirstNumber/$SecondNumber = " +
		$([math]::Round([float]$FirstNumber/[float]$SecondNumber,2)))
}
...
if ($FirstNumber -lt 0)
{
	Write-Warning $("You have entered a negative number. " +
		"Unable to calculate its square root, log base 10, and natural logarithm")
}
...
{
	Write-Host $("You have entered a positive number. Calculating square root, log base 10, " +
		"and natural logarithm") -ForegroundColor Green
